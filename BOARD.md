...
- **T001** — Разложить `CONCEPT.md` по живой проектной документации
  (`README.md` + `DECISIONS.md` + `BACKLOG.md`). Ветка:
  `T001-decompose-concept`.

  Acceptance:
  - `README.md` расширен: цель проекта, 3 принципа (из заголовка
    концепта), диаграмма сквозного пайплайна (раздел 1), таблица
    «соотношение готового и своего» (раздел 18).
  - `DECISIONS.md` содержит ADR на:
    - архитектурный принцип «MCP-обвязка готовых инструментов,
      минимум собственного кода»;
    - выбор каждого ключевого компонента стека (kicad-mcp-pro,
      kicad-sch-api, SPICEBridge, PyOpenMagnetics+FEMM,
      FreeCAD+freecad-mcp) — каждый ADR с разделом «Альтернативы
      и почему отвергли»;
    - стратегия версионирования зависимостей (раздел 19 концепта).
  - `BACKLOG.md` наполнен задачами фаз 1a/1b/2/3/4 дорожной карты
    (раздел 13 концепта) — каждая фаза = несколько T-задач (T002+),
    с короткими acceptance criteria.
