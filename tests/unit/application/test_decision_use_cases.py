"""Application use cases for Decision (T099 Phase 2 + T157)."""
...
def _setup_project(tmp_path: Path, name: str = 'p') -> tuple[Project, Path]:
    project_path = tmp_path / name
    project_path.mkdir()
    return Project(name=name, path=project_path), project_path


async def test_add_decision_writes_markdown_and_updates_manifest(
    tmp_path: Path,
) -> None:
    ...
    saved = await manifest_repo.load(project.path)
    assert len(saved.decisions) == 1
    assert saved.decisions[0].id == 'D001'


async def test_add_decision_auto_increments_id(tmp_path: Path) -> None:
    project, _ = _setup_project(tmp_path)
    manifest_repo = FakeManifestRepository(project)
    decision_repo = FakeDecisionRepository()
...
async def test_add_decision_unknown_project_raises(tmp_path: Path) -> None:
...
async def test_add_decision_manifest_save_failure_raises_decision_persistence(
    tmp_path: Path,
) -> None:
    project, _ = _setup_project(tmp_path)
...
async def test_list_decisions_returns_from_markdown(tmp_path: Path) -> None:
    project, _ = _setup_project(tmp_path)
    manifest_repo = FakeManifestRepository(project)
    decisions = [
        Decision(
...
async def test_get_decision_returns_loaded(tmp_path: Path) -> None:
    project, _ = _setup_project(tmp_path)
    manifest_repo = FakeManifestRepository(project)
    target = Decision(
        id='D003',
...
async def test_get_decision_missing_raises(tmp_path: Path) -> None:
    project, _ = _setup_project(tmp_path)
    manifest_repo = FakeManifestRepository(project)
    decision_repo = FakeDecisionRepository()
