"""Snapshot-тест: data/templates/active-lpf-sallen-key/ — sync с builder'ом (T027 D)."""
...
_TEMPLATE_DIR = _REPO_ROOT / 'data' / 'templates' / 'active-lpf-sallen-key'
...
def fresh_bake_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    work = tmp_path_factory.mktemp('regen-active-lpf-sallen-key')
    ...
    target = work / 'active-lpf-sallen-key'
    module._reseed_uuid_rng_for_template('active-lpf-sallen-key')  # noqa: SLF001
    module._bake_active_lpf_sallen_key(target)  # noqa: SLF001
    return target


def test_active_lpf_sallen_key_template_in_sync_with_builder(
    fresh_bake_dir: Path,
) -> None:
