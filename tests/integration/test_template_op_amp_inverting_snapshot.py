"""Snapshot-тест: data/templates/op-amp-inverting/ — sync с builder'ом (T153 C.1.2).

Аналог `test_template_nfb_se_amp_snapshot.py` для op-amp-inverting
calibration reference (T153 Phase C.1). Пересобирает шаблон в tmp
каталог через `_bake_op_amp_inverting`, нормализует UUID и сверяет
с запечённым в `data/templates/op-amp-inverting/`.
"""
...
_TEMPLATE_DIR = _REPO_ROOT / 'data' / 'templates' / 'op-amp-inverting'
...
def fresh_bake_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    work = tmp_path_factory.mktemp('regen-op-amp-inverting')
    ...
    target = work / 'op-amp-inverting'
    # Reseed UUID RNG per template (как в main()):
    module._reseed_uuid_rng_for_template('op-amp-inverting')  # noqa: SLF001
    module._bake_op_amp_inverting(target)  # noqa: SLF001
    return target


def test_op_amp_inverting_template_in_sync_with_builder(
    fresh_bake_dir: Path,
) -> None:
