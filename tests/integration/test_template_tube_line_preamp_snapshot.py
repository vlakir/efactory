"""Snapshot-тест: data/templates/tube-line-preamp/ — sync с builder'ом (T027 B).

Аналог `test_template_tube_pp_amp_snapshot.py` для tube-line-preamp
template (T027 Phase B).
"""
...
_TEMPLATE_DIR = _REPO_ROOT / 'data' / 'templates' / 'tube-line-preamp'
...
def fresh_bake_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    work = tmp_path_factory.mktemp('regen-tube-line-preamp')
    ...
    target = work / 'tube-line-preamp'
    module._reseed_uuid_rng_for_template('tube-line-preamp')  # noqa: SLF001
    module._bake_tube_line_preamp(target)  # noqa: SLF001
    return target


def test_tube_line_preamp_template_in_sync_with_builder(
    fresh_bake_dir: Path,
) -> None:
