"""Snapshot-тест: data/templates/nfb-se-amp/ — sync с builder'ом (T153 A.2).

Аналог `test_template_se_amp_snapshot.py` для NFB-варианта (двухкаскадный
6Н1П + 6П14П с global feedback). Пересобирает шаблон в tmp каталог
через `_bake_nfb_se_amp`, нормализует UUID и сверяет с запечённым в
`data/templates/nfb-se-amp/`.
"""
...
_TEMPLATE_DIR = _REPO_ROOT / 'data' / 'templates' / 'nfb-se-amp'
...
def fresh_bake_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    work = tmp_path_factory.mktemp('regen-nfb')
    ...
    target = work / 'nfb-se-amp'
    module._bake_nfb_se_amp(target)  # noqa: SLF001
    return target


def test_nfb_se_amp_template_in_sync_with_builder(
    fresh_bake_dir: Path,
) -> None:
