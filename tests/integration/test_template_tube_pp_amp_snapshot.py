"""Snapshot-тест: data/templates/tube-pp-amp/ — sync с builder'ом (T027 A).

Аналог `test_template_nfb_se_amp_snapshot.py` для tube-pp-amp template
(T027 Phase A). Пересобирает шаблон в tmp каталог через
`_bake_tube_pp_amp`, нормализует UUID и сверяет с запечённым в
`data/templates/tube-pp-amp/`.
"""
...
_TEMPLATE_DIR = _REPO_ROOT / 'data' / 'templates' / 'tube-pp-amp'
...
def fresh_bake_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    work = tmp_path_factory.mktemp('regen-tube-pp-amp')
    ...
    target = work / 'tube-pp-amp'
    module._reseed_uuid_rng_for_template('tube-pp-amp')  # noqa: SLF001
    module._bake_tube_pp_amp(target)  # noqa: SLF001
    return target


def test_tube_pp_amp_template_in_sync_with_builder(
    fresh_bake_dir: Path,
) -> None:
