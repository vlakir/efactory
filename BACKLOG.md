...
- **T185** — [2026-06-04, discovered during T182 Phase 4]
  **Re-vision Mullard EL34 knee region — denser sampling.**
  Текущая `el34_mullard.json` fixture (36 точек, T031 Phase 0) имеет
  только 3 точки в SC#2-определяющей knee region (Va<150 ∧
  Vg∈[-10,-20]), что недостаточно для статистики acceptance. Phase 4
  показал knee mean 39% (target 30%) — gap может быть вызван
  малостью выборки. Re-vision Mullard EL34 Page C2 с focus на
  Va∈[20, 150] и Vg∈[-5, -20] — целевые 8-12 точек в knee region.

  **Acceptance:**
  - 8+ vision-extracted точек в Va<150 ∧ Vg∈[-5, -20].
  - Re-prove T182 modified-knee SC#2 на denser fixture: knee mean
    должен спуститься к target <30%, max <40%.
  - Если не закрывает — это direct evidence что Tier 2/1 нужен
    (T184).

- **T184** — [2026-06-04, discovered during T182 Phase 4]
  **Pilot Reefman 2003 (Tier 2) extended Koren formulation для
  tube-fitting.** T182 Phase 4 показал, что modified Koren+sigma
  упирается в structural ceiling (~30-40% knee mean на EL34) — full
  closure до <20% требует более параметрической formulation.
  Reefman 2003 «Improved SPICE models for pentode tubes» (AES
  paper) добавляет 3-4 параметра к canonical Koren (grid-coupling
  + cathode-warm-up + saturation) с claim ±10-15% accuracy.
  Pilot — single-tube probe (EL34) с Reefman fit, comparison с
  T182 modified-knee.

  **Acceptance:**
  - Reefman forward formula реализована в `_formulas.py` за
    отдельным variant `koren-reefman` (third FormulaVariant).
  - Single-tube probe Mullard EL34: knee mean <20%, plateau <10%.
  - .lib emission ngspice-portable.
  - DECISIONS ADR с comparison vs T182 modified-knee (cost/benefit).

  **Контекст.** `phase-4-acceptance.md` §6 follow-ups, ADR-T182a
  ROI matrix (Reefman rejected baseline → revisit при T182 ceiling).

- **T183** — [2026-06-04, discovered during T182 Phase 4]
  **Add relative-error sigma weighting в canonical T031 fitter.**
  T182 Phase 4 key finding: бóльшая часть EL34 knee улучшения (146% →
  39% mean) приходит от добавленного в modified fitter `σ = max(Ia,
  1 mA)` weighting, не от `(1-exp(-Va/Vk))` modifier. Применение
  того же sigma weighting в `fit_koren_triode` / `fit_ayumi_pentode`
  должно дать сравнимое улучшение без switch на modified variant.

  **Acceptance:**
  - `fit_koren_triode` / `fit_ayumi_pentode` принимают опциональный
    `relative_weights: bool` параметр (default `False` —
    backwards-compat для T031 acceptance baseline).
  - При `relative_weights=True` загружают σ = max(Ia, 1 mA) и
    absolute_sigma=False в curve_fit.
  - SC#1 round-trip 12AX7 / Ayumi-EL34 сохраняется при обоих режимах.
  - Probe-script (`scripts/t183-canonical-sigma-comparison.py`)
    демонстрирует EL34 knee mean drop ~146% → ~39% при canonical+σ
    (Phase 4 manual experiment подтвердил эту цифру).
  - CLI flag `--relative-weights` опционально.

  **Контекст.** `specs/T182-koren-modified-knee/phase-4-acceptance.md`
  §2 «Key finding» — sigma alone closes ~75% gap; modifier добавляет
  marginal.
