...
- **T095** — [2026-05-17] Auto-install pre-commit hook через
  `uv sync` (post-install). Сейчас после клонирования требуется
  ручной `uv run pre-commit install --hook-type pre-push` — новый
  разработчик может пропустить, гейт молча не сработает.
  Acceptance: после первого `uv sync` хук установлен автоматически
  (через post-install script / `pyproject.toml` hook / альтернативу
  без выхода за рамки `uv`); README обновлён; на чистом клоне
  поведение проверено вручную. Решение по механизму записать в
  `DECISIONS.md` (ADR), т.к. это процессное изменение.
  Источник: ретро `[0.2.0]`.
