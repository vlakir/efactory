...
3. **User overlay** — изначально (A) только built-in; **пересмотрено
   2026-05-18 на (B)** по запросу пользователя сразу после merge T006:
   adapter принимает второй `user_tube_library_root` (default
   `<storage_root>/models/tubes/`, env
   `EFACTORY_USER_TUBE_LIBRARY_ROOT`). Сначала сканируется built-in,
   потом user; user-id'ы перезаписывают built-in (overlay). Это
   закрывает dev/end-user gap до T002 bootstrap. Реализовано в
   fix-up `fixes/T006-user-overlay`.
