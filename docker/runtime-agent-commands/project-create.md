...
5. **T025 auto-show схемы.** Просканируй stdout на строки вида
   ...
   **`xdg-open <abs path> &`** через Bash — `eog` (Eye of
   GNOME) откроется на host через X11 forwarding, пользователь
   увидит реальную схему в отдельном окне. `&` отправляет
   процесс в background, terminal сразу освобождается.

   Запомни эти пути — если позже пользователь скажет «покажи
   схему», «открой схему», «как выглядит» и т.п. — запусти
   `xdg-open` на сохранённом пути.
