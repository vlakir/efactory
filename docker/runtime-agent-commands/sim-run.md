...
5. **T025 auto-show схемы.** Просканируй stdout на строки вида
   ...
   Для каждой такой строки запусти **`xdg-open <abs path> &`**
   через Bash — `eog` (Eye of GNOME) откроется на host через X11
   ...
   схему», «открой схему ещё раз» и т.п. — запусти `xdg-open`
   на том же пути.
